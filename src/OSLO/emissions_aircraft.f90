...
! trop chem will remove chemical H2O at surface 
  ...
  subroutine aircraft_emis_update()
    ...
    if (trim(AirScen).eq.'NONE') then
       ...
       dz_ac  = dft_ac*ft2km
       actual_levels = 25
    else if (trim(AirScen).eq.'CEDS2021') then
       !// CEDS 2021 emissions
       dft_ac = 2000._r8
       dz_ac  = dft_ac*ft2km
       actual_levels = 25
...
!$omp end parallel
  ...
  subroutine read_original_res()
    ...
    if (AirScen(1:8) .eq. 'TradeOff') then
       ...
       call read_ceds_2017()
    else if (AirScen(1:8) .eq. 'CEDS2021') then
       call read_ceds_2021()
...
!$omp end parallel
  ...
  subroutine read_ceds_2021()
    ...
    !// Amund Sovde Haslerud, October 2017
    !// MTL 07/01/17
    !// RBS 12/08/21 copied the 2017 routine
    !// --------------------------------------------------------------------
    use cmn_met, only: MYEAR
    use cmn_parameters, only: A0, CPI180
    use ncutils, only: get_netcdf_var_1d, readnc_3d_from4d
    ...
    integer, dimension(12), parameter :: midmonth = &
         (/15, 45, 74, 105, 135, 166, 196, 227, 258, 288, 319, 349 /)
    !// --------------------------------------------------------------------
    character(len=*), parameter :: subr = 'read_ceds_2021'
    !// --------------------------------------------------------------------
    ...
    else if (getY .eq. 2020) then
       infileyear = '202001-202012'
       version ='2021-04-21_covid_modified_v5_1'
       start_year = 2015
    endif
