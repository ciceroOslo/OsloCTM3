...
contains
  ...
  subroutine dump3hrs(NDAYI,NDAY,NMET)
    ...
    if(LDUMP3HRS(2)) then
       ...
       TMPFIELD(:,:,:,1) = AIR(:,:,:)
       call gm_dump_nc_perm2(1, 'air_density', TMPFIELD(:,:,:,1), 1, (/1/), &
            NMET, 1, (/1/), time_label, ncfilename)

       ncfilename = 'met'//cyear//'_'//cday//'.nc'
       TMPFIELD(:,:,:,1) = T(:,:,:)
       call gm_dump_nc_org(1, 'temperature', TMPFIELD(:,:,:,1), 1, (/1/), &
            NMET, 1, (/1/), time_label, ncfilename)

       ncfilename = 'zgh'//cyear//'_'//cday//'.nc'
       TMPFIELD(:,:,:,1) = T(:,:,:) !will not be used
       call gm_dump_nc_height(1,'geopothght', TMPFIELD(:,:,:,1), 1, (/1/), &
            NMET, 1, (/1/), time_label, ncfilename)
       !ncfilename = 'ps0'//cyear//'_'//cday//'.nc'
       !TMPFIELD(:,:,1,1) = P(:,:,:)
       !call gm_dump_nc_org_2d(1, 'temperature', TMPFIELD(:,:,1,1), 1, (/1/), &
       !     NMET, 1, (/1/), time_label, ncfilename)
    ...
    if (LOSLOCTROP .and. LDUMP3HRS(3)) then
       ...
       call gm_dump_nc_perkg(NPAR, GMNAME, STT, TRACER_ID_MAX, trsp_idx, &
            NMET, trp_nr, TRP_LIST, time_label, ncfilename)

       !//Legge inn en ekstra liste for mtr her.
       ncfilename = 'mtr'//cyear//'_'//cday//'.nc'
       TMPFIELD(:,:,:,1) = STT(:,:,:,trsp_idx(mtr_list(1)))
       do N = 2, mtr_nr
          TMPFIELD(:,:,:,1) = TMPFIELD(:,:,:,1) &
               + STT(:,:,:,trsp_idx(mtr_list(N)))
       end do
       call gm_dump_nc_perkg(2, (/'MNT       '/), TMPFIELD, 1, (/1/), &
            NMET, 1, (/1/), time_label, ncfilename)
    ...
    if (LOSLOCTROP .and. LDUMP3HRS(13)) then
       ...
       call gm_dump_nc_ntr(NOTRPAR, XTNAME, XSTT, TRACER_ID_MAX, Xtrsp_idx, &
  ...
  subroutine gm_dump_nc_height( &
    ...
    integer, parameter :: nc4shuffle=1
    !// --------------------------------------------------------------------
    character(len=*), parameter :: subr = 'gm_dump_nc_height'
    !// --------------------------------------------------------------------
    ...
    if (nbr_steps .eq. 1) then
       ...
       DO N = 1, NCOMP
          ...
          !// Add text descriptions and units
          status = nf90_put_att(ncid,TCINFO(N,2),'units','m')        
          if (status/=nf90_noerr) call handle_error(status,'attribute units tracer')
       ...
       do L = 1, LPAR
          do J = 1, JPAR
             do I = 1, IPAR
                ...
                !!// Field for component ICOMPLIST(N)
                RTCDMP(I,J,L) = 0.5_r8*(ZOFLE(L+1,I,J)+ZOFLE(L,I,J)) !m
             end do
          end do
       end do
    ...
    !// --------------------------------------------------------------------
  end subroutine gm_dump_nc_height



  !Orginal, without dividing by volume or area
  !// ----------------------------------------------------------------------
  subroutine gm_dump_nc_org( &
       NPAR_IN,     & !I Number of components in STT_IN (not necessarily STT)
       TNAME_IN,    & !I Name of all the tracers sent in
       STT_IN,      & !I 4-D array with components
       IREPMX,      & !I Number of chemical components in MTC_IN
    ...
    integer, parameter :: nc4shuffle=1
    !// --------------------------------------------------------------------
    character(len=*), parameter :: subr = 'gm_dump_nc_org'
    !// --------------------------------------------------------------------
    ...
    DO N = 1, NCOMP
       ...
       !         !!// Field for component ICOMPLIST(N)
       RTCDMP(:,:,:) = STT_IN(:,:,:,TCINFO(N,1))  !1.e3_r8   &                         !kg-->g
       !              *STT_IN(I,J,L,TCINFO(N,1))  &                  !kg
       !              /(AREAXY(I,J) * (ZOFLE(L+1,I,J)-ZOFLE(L,I,J))) !m^3
       !      end do
       !   end do
    ...
    !// --------------------------------------------------------------------
  end subroutine gm_dump_nc_org


!//RBS Add routine for the non-transported components
  

  !// ----------------------------------------------------------------------
  subroutine gm_dump_nc_ntr( &
       NPAR_IN,     & !I Number of components in STT_IN (not necessarily STT)
       TNAME_IN,    & !I Name of all the tracers sent in
       STT_IN,      & !I 4-D array with components
       IREPMX,      & !I Number of chemical components in MTC_IN
    ...
    integer, intent(in)           :: NPAR_IN                         !Number of components
    character(len=10),intent(in)  :: TNAME_IN(NPAR_IN)               !Tracer names
    real(r8), intent(in)          :: STT_IN(LPAR,NPAR_IN,IPAR,JPAR)  !4-D fields
    integer, intent(in)           :: NCOMP                           !Number of components in list
    integer, intent(in)           :: ICOMP_LIST(NCOMP)               !Numbers of comps to put out
    integer, intent(in)           :: IREPMX                          !Number of species in MTC_IN
    integer,intent(in)            :: MTC_IN(IREPMX)                  !Comp.order (not necessarily MTC)
...
!// ----------------------------------------------------------------------
  subroutine gm_dump_nc_perkg( &
    ...
    use cmn_precision, only: r8
    use cmn_size, only: IPAR, JPAR, LPAR
    use cmn_ctm, only:  AREAXY, XDGRD, YDGRD, ZGRD, NRMETD, AIR
    use cmn_met, only:  ZOFLE
    use ncutils, only:  handle_error
    !// --------------------------------------------------------------------
    implicit none
    ...
    integer, parameter :: nc4shuffle=1
    !// --------------------------------------------------------------------
    character(len=*), parameter :: subr = 'gm_dump_nc_perkg'
    !// --------------------------------------------------------------------
    ...
    if (nbr_steps .eq. 1) then
       ...
       DO N = 1, NCOMP
          ...
          !// Add text descriptions and units
          status = nf90_put_att(ncid,TCINFO(N,2),'units','kg kg-1')        
          if (status/=nf90_noerr) call handle_error(status,'attribute units tracer')
       ...
       do L = 1, LPAR
          do J = 1, JPAR
             do I = 1, IPAR
                ...
                !!// Field for component ICOMPLIST(N)
                !// CHANGED HERE SO THE UNIT IS KG PER KG
                RTCDMP(I,J,L) = STT_IN(I,J,L,TCINFO(N,1))  &                  !kg
                     /AIR(I,J,L) !  kg /kg  #(AREAXY(I,J) * (ZOFLE(L+1,I,J)-ZOFLE(L,I,J))) !m^3
             end do
          end do
       end do
    ...
    !// --------------------------------------------------------------------
  end subroutine gm_dump_nc_perkg
  !// ----------------------------------------------------------------------

  subroutine gm_dump_nc_perm2( &
       NPAR_IN,     & !I Number of components in STT_IN (not necessarily STT)
       TNAME_IN,    & !I Name of all the tracers sent in
       STT_IN,      & !I 4-D array with components
       IREPMX,      & !I Number of chemical components in MTC_IN
    ...
    integer, parameter :: nc4shuffle=1
    !// --------------------------------------------------------------------
    character(len=*), parameter :: subr = 'gm_dump_nc_perm2'
    !// --------------------------------------------------------------------
    ...
    if (nbr_steps .eq. 1) then
       ...
       DO N = 1, NCOMP
          ...
          !// Add text descriptions and units
          status = nf90_put_att(ncid,TCINFO(N,2),'units','kg m-2')        
          if (status/=nf90_noerr) call handle_error(status,'attribute units tracer')
    ...
    !// --------------------------------------------------------------------
  end subroutine gm_dump_nc_perm2
