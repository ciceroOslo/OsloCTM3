...
module tropchem_oslo
  !// ----------------------------------------------------------------------
  !// MODULE: chem_oslo
  !// DECRIPTION: Routine for driving the Oslo chemistry routines
  !//
  !// Contains:
  !//   subroutine oslochem
  !//
...
contains
  ...
  !// ----------------------------------------------------------------------
  subroutine oslochem(BTT,BJV,BAIR_MOLEC,BVOL,BTEM,AIRB,BEMIS,DTCHM,MP)
    !// --------------------------------------------------------------------
    ...
    use cmn_fastjx, only: JVN_
    use cmn_met, only: P, Q, CLDFR, CLDLWC, CLDIWC, PRECLS
    use cmn_sfc, only: VDEP
    use cmn_oslo, only: LMTROP, PR42HET, MTC, IDMTC, XMTC, XIDMTC, XTMASS, &
         XSTT, LVS2ADD2TC, TROPCHEMnegO3
    use cmn_parameters, only: M_AIR, AVOGNR, R_AIR
    use chem_oslo_rates, only: TEMPRANGE, MINTEMP, TCRATE_TP_IJ, TCRATE_onAER
    ...
    use soa_oslo, only: SOA_v9_separate, soa_diag_drydep, &
         soa_diag_separate
    use diagnostics_general, only: nchemdiag, save_chemPL
    ...
    real(r8), dimension(LPAR) :: &
          ...
          r_c2h2_oh        !C2H2 + OH + M -> 0.65GLYOXAL + 0.65OH + 0.35HCOOH + 0.35HO2 + 0.35*CO + M
    ...
    do J = MPBLKJB(MP), MPBLKJE(MP)
      ...
      do I = MPBLKIB(MP), MPBLKIE(MP)
        ...
        call gotoZC_IJ(ZC_LOCAL,BTT,XSTT,I,J,II,JJ,1,LMTP, &
             ...
             NPAR, NOTRPAR, TRACER_ID_MAX, IDMTC, XIDMTC)
        ...
        call ZC_MASS2CONC(ZC_LOCAL,1,LMTP,LPAR,NPAR,NOTRPAR, &
             TRACER_ID_MAX, MTC, XMTC,TMASS,XTMASS,DV,AVOGNR)



        !// Calculate rates dependent on T & P
        Call TCRATE_TP_IJ(I, J, LPAR, LMTP, &
             TEMP, AIR_MOLEC, H2O_MOLEC, PMIDL, &
             r_op_o2_m, r_ho2_ho2_tot, r_oh_oh_m, r_no2_oh_m, &
             r_no2_no3_m, r_n2o5_m, r_ho2_no2_m, r_ho2no2_m, r_oh_hno3, &
